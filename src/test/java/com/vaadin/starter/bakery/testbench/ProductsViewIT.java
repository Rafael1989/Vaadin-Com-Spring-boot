...
public class ProductsViewIT extends AbstractIT<ProductsViewElement> {
	...
	public void editProduct() {
		...
		ComboBoxElement status = productsPage.getStatus();
		String newStatus = toggleStatus(status);
		...
		status = productsPage.getStatus();
		Assert.assertEquals(newStatus, status.getSelectedText());
	...
	private String toggleStatus(ComboBoxElement status) {
		String currentStatus = status.getSelectedText();
		String newStatus;
		if(ProductStatus.OUT_OF_STOCK.getDescription().equals(currentStatus)){
			newStatus = ProductStatus.IN_STOCK.getDescription();
		}else{
			newStatus = ProductStatus.OUT_OF_STOCK.getDescription();
		}
		status.selectByText(newStatus);
		return newStatus;
	}
